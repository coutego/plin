...
- **File-based persistence** (EDN file) for data storage
...
| ~todo-node-server.plugins.disk-store~ | File-based EDN storage | [[file:src/todo_node_server/plugins/disk_store.cljs][plugins/disk_store.cljs]] |
...
Like all examples, this uses the shared persistence interface from the common module. This plugin overrides ~::load-fn~ and ~::store-fn~ with file-based EDN storage implementations. See the [[file:../common/src/todo/plugins/persistence.cljc][persistence plugin]] for details.
...
#+begin_src shell
npx nbb -cp "src:../shared/src" src/todo_node_server/main.cljs
#+end_src
...
- Data persists to EDN file and survives server restarts
...
Uses a file-based EDN storage mechanism to persist the ~TaskList~ to disk. Data is automatically saved on every change and loaded on startup, ensuring persistence across server restarts. The storage file is located at ~data/todos.edn~.
