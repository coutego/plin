(ns todo-node-server.plugins.http-server
  "HTTP server plugin for Node.js using built-in http module.
...
(defn task-html
  ...
  (let [status (due-date-status task)
    ...
    (str "<div class='task " status-class "'>"
         ...
         "<input type='checkbox' " (if (:completed task) "checked" "") " onChange='this.form.submit()' />"
         "</form>"
...
(defn task-list-html
  ...
  (str "<h1>Todo List</h1>
        ...
        <div class='filters'>
          <a href='/?filter=all'" (if (= filter-param "all") " class='active'" "") ">All</a>
          <a href='/?filter=active'" (if (= filter-param "active") " class='active'" "") ">Active</a>
          <a href='/?filter=overdue'" (if (= filter-param "overdue") " class='active'" "") ">Overdue</a>
          <a href='/?filter=completed'" (if (= filter-param "completed") " class='active'" "") ">Completed</a>
        </div>
        <div class='tasks'>"
...
;; =============================================================================
;; Route Handlers (dependencies first, request-related args last)
;; =============================================================================
...
(defn home-handler
  ...
  [load-fn get-all-tasks get-active-tasks get-completed-tasks
   get-overdue-tasks sort-by-urgency task-html-fn res query]
  (let [filter-param (or (.-filter query) "all")
        task-list (load-fn)
    ...
    (.end res (html-page (task-list-html task-html-fn sorted-tasks
...
(defn calendar-handler
  ...
  [load-fn calendar-data-fn render-calendar-fn render-task-fn
   format-due-date status-label due-date-status res]
...
(defn set-due-date-handler
  ...
  [load-fn store-fn set-due-date parse-date req res id-str]
  (let [body (atom "")]
    (.on req "data" (fn [chunk] (swap! body str chunk)))
    (.on req "end" (fn []
                     (let [params (parse-body @body)
                           task-id (js/parseInt id-str)
                           due-date-str (:due-date params)
                           due-date (parse-date due-date-str)
                           task-list (load-fn)
...
(defn clear-completed-handler
  ...
  [load-fn store-fn clear-completed res]
  (let [task-list (load-fn)
...
(defn make-router
  ...
  [home-handler calendar-handler create-handler toggle-handler
   delete-handler due-date-handler clear-handler]
  (fn [req res]
    (let [req-url (.-url req)
          parsed-url (url/parse req-url true)
          pathname (.-pathname parsed-url)
          query (.-query parsed-url)
          method (.-method req)]
      (cond
        ;; Home page - GET /
        (and (= pathname "/") (= method "GET"))
        (home-handler res query)

        ;; Calendar view - GET /calendar
        (and (= pathname "/calendar") (= method "GET"))
        (calendar-handler res)

        ;; Create task - POST /tasks
        (and (= pathname "/tasks") (= method "POST"))
        (create-handler req res)

        ;; Clear completed - POST /tasks/clear-completed (must come before generic route)
        (and (= pathname "/tasks/clear-completed") (= method "POST"))
        (clear-handler res)

        ;; Task actions - POST /tasks/:id/:action
        (and (str/starts-with? pathname "/tasks/") (= method "POST"))
        (let [parts (str/split pathname #"/")
              id-str (nth parts 2 nil)
              action (nth parts 3 nil)]
          (case action
            "toggle" (toggle-handler req res id-str)
            "delete" (delete-handler req res id-str)
            "due-date" (due-date-handler req res id-str)
...
(defn make-start-server
  "Factory: creates start-server function with injected router."
  [router]
  (fn [port]
    (let [server (.createServer http router)]
