...
| ~todo-node-tui.plugins.disk-store~ | File-based EDN storage | [[file:src/todo_node_tui/plugins/disk_store.cljs][plugins/disk_store.cljs]] |
...
# Run (uses shared dependencies from ../shared/src/)
npx nbb -cp "src:../shared/src" src/todo_node_tui/main.cljs
#+END_SRC
...
Uses a file-based EDN storage mechanism to persist the ~TaskList~ to disk. Data is automatically saved on every change and loaded on startup, ensuring persistence across application restarts. The storage file is located at ~data/todos.edn~.
